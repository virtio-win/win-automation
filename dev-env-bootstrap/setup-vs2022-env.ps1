<#
.SYNOPSIS
    Setup script for VS 2022 Community + WDK + WinFSP + CPDK development environment.
...
$tempDir = Join-Path $env:TEMP "vs2022-setup"
...
# ---------------------------------------------
Write-Section "Visual Studio 2022 Community Edition"

# Embedded VS 2022 component configuration
$vsConfigJson = @'
{
  "version": "1.0",
  "components": [
...
$vsConfigPath = Join-Path $tempDir "vs2022.vsconfig"
Set-Content -Path $vsConfigPath -Value $vsConfigJson -Encoding UTF8

# Detect existing VS 2022 via vswhere
$vswhere = "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe"
$vsInstalled = $false
$vsInstallPath = $null

if (Test-Path $vswhere) {
    $vsInstallPath = & $vswhere -version "[17.0,18.0)" -products Microsoft.VisualStudio.Product.Community -property installationPath 2>$null | Select-Object -First 1
    if ($vsInstallPath) {
        $vsInstalled = $true
        $vsVersion = & $vswhere -version "[17.0,18.0)" -products Microsoft.VisualStudio.Product.Community -property installationVersion 2>$null | Select-Object -First 1
        Write-Status "VS 2022 Community is installed (v$vsVersion): $vsInstallPath"
    }
}

if (-not $vsInstalled) {
    Write-Host "[MISS]  VS 2022 Community is not installed." -ForegroundColor Yellow
}

$vsSetupUrl = "https://c2rsetup.officeapps.live.com/c2r/downloadVS.aspx?sku=community&channel=Release&version=VS2022"
...
if ($vsInstalled) {
    ...
    if ($script:NoPrompt) {
        ...
        $answer = Read-Host "  Update VS 2022 and ensure all required components are installed? (Y/N)"
        if ($answer -match '^[Yy]') { $doUpdate = $true }
    }

    if ($doUpdate) {
        try {
            Get-Installer -Url $vsSetupUrl -OutFile $vsSetupExe -DisplayName "VS 2022 bootstrapper"
            ...
            Write-Err "VS 2022 update/modify failed: $_"
        }
    }
} else {
    $doInstall = $false
    if ($script:NoPrompt) {
        ...
        $answer = Read-Host "  Install VS 2022 Community Edition? (Y/N)"
    ...
    if ($doInstall) {
        try {
            ...
            $exitCode = Start-InstallerWithProgress -FilePath $vsSetupExe `
                ...
                -StepName "VS 2022 install"
            if ($exitCode -ne 0 -and $exitCode -ne 3010) {
                throw "VS installer exited with code $exitCode"
            }
            Write-Status "VS 2022 Community installed successfully."
            if ($exitCode -eq 3010) {
                Write-Warn "A system restart is required to complete VS 2022 installation."
            }
        } catch {
            Write-Err "VS 2022 installation failed: $_"
        }
    } else {
        Write-Warn "VS 2022 Community not installed."
    }
}
