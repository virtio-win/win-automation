...
.DESCRIPTION
    Checks for and installs:
      ...
      - Windows Driver Kit (WDK) build 28000
...
if ($vsInstalled) {
    ...
    if ($doUpdate) {
        try {
            ...
            Assert-InstallerSuccess -ExitCode $exitCode -StepName "VS 2026 update"
            ...
            Assert-InstallerSuccess -ExitCode $exitCode -StepName "VS 2026 modify"
...
# ---------------------------------------------
# 2. Windows Driver Kit (WDK) build 28000
# ---------------------------------------------
Write-Section "Windows Driver Kit (WDK) build 28000"

$wdkInstalled = $false
$wdkKitsRoot = "${env:ProgramFiles(x86)}\Windows Kits\10"
if (Test-Path -LiteralPath $wdkKitsRoot -PathType Container) {
    $wdkInstalled = Test-WdkInstallation -KitsRoot $wdkKitsRoot -RequiredBuild "28000"
    ...
    if ($wdkInstalled) {
        ...
        Write-Status "WDK build 28000 is installed (available versions: $wdkVersions)"
    }
}
...
if (-not $wdkInstalled) {
    ...
    if ($doInstall) {
        ...
        try {
            Get-Installer -Url $wdkUrl -OutFile $wdkSetupExe -DisplayName "WDK build 28000 Setup"
