...
Both shapes are accepted. For `apps`, the whole-list flag disables both the main installation step and new first-login scheduling; it does not remove a task registered by an earlier run. Per-entry `disabled: true` is supported for `defender_exclusions`, `smb_shares`, `local_users`, and `apps`; it is not supported for `root_ca` entries.
